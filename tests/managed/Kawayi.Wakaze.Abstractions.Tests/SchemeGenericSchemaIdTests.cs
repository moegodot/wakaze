...
public class SchemeGenericSchemaIdTests
{
    ...
    {
        var matches = SchemaId<SemanticScheme>.TryParse("semantic://wakaze.dev/tag/v2", out var matching);
        var mismatched = SchemaId<SemanticScheme>.TryParse("database://wakaze.dev/postgresql/v1", out _);
    ...
    public async Task ExplicitConversion_FromUntyped_Preserves_Value()
    {
        ...
        await Assert.That(schema).IsEqualTo(new SchemaId<DatabaseScheme>(PostgreSqlFamily.Family, new SchemaVersion(1)));
    }
